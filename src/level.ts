...
class Tags {
    ...
    get_tags(){
        ...
        for(let i=0;i<this.tag_names.length;i++){
            if(bits[i]){
                const tag_name = this.tag_names[i]
                if(tag_name) tags.push(tag_name)
            }
        }
        return tags.join(',')
    }
...
interface CellObjCtorVal {
    val:number
}
interface CellObjCtorComponents {
    base:number,
    paint:number
}
type CellObjCtorArg = CellObjCtorVal|CellObjCtorComponents
class CellObj{
    _val:number
    constructor(arg?:CellObjCtorArg) {
        if(!arg){
            this._val = 0
            return
        }
    ...
    _normalizedCellValue(val:number){
    ...
    _normalizedPaint(val:number){
        return this._normalizedCellValue(val)&0xffffff80
    }
    _normalizedBase(val:number){
        return this._normalizedCellValue(val)&0x7f
    }

    get value(){return this._val}
    set value(val:number){this._val = this._normalizedCellValue(val)}
...
class Grid {
    ...
    setCellBase(x:number,y:number,val:number){
        this._checkLimits(x,y)
        const cell = new CellObj({val:this.celldata[y][x]})
        cell.base = val
        this.celldata[y][x]=cell.value
    }

    setCellPaint(x:number,y:number,val:number){
        this._checkLimits(x,y)
        const cell = new CellObj({val:this.celldata[y][x]})
        cell.paint = val
        this.celldata[y][x]=cell.value
...
class CalloutList{
    ...
    deserialize(buf:Buffer, offset:number){
        ...
        for(let i=0;i<n_callouts;i++){
            ...
            offset+=4
            const text = buf.slice(offset,offset+n-1).toString('utf-8') // leave out the trailing zero byte.
            offset+=n
            this.callouts.push({x,y,text})
        }
