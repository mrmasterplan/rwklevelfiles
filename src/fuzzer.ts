    ...
    static anything_to_fuzz(){
        for(let fuzz_file of glob.sync(config.fuzzer.fuzz_files)){
            for(let i of JSON.parse(fs.readFileSync(fuzz_file,'utf-8'))){
                ...
                if (! fs.existsSync(tile_file)) return true;
            }
        }
        if (! fs.existsSync(`${config.fuzzer.tiles}/robot.png`)) return true;
        if (! fs.existsSync(`${config.fuzzer.tiles}/kitty.png`)) return true;
        return false
    }
