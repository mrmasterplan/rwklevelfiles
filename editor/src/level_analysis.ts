...
const level_structure = {
    header: {
        name_section_size_start:0x0c,
        name_section_size_add:24,
        name_size_start:0x14,
        name_start: 0x18,
        cell_data_field_size: 0x2c, //plus name length
...
export class LevelDetails {
    ...
    constructor(public inbuf:Buffer) {
        ...
        console.log(`extracted grid ${this.grid.rows},${this.grid.cols}`)
        this.pgrid = this._extractPostGrid()
        console.log('extracted post-grid')
        console.log(`next offset: ${this.pgrid.offset+this.pgrid.buf.length}`)
        this.callouts = new CalloutList(inbuf,this.pgrid.offset+this.pgrid.buf.length)
        console.log('extracted callouts')
    }
...
export class Level_analysis {
    ...
    setName(buf:Buffer,name:string){
        ...
        const newbuf = Buffer.concat([buf_before,name_buf,buf_after])
        //now we need to fix the size of the name field
        // and the size of the overall name section
        newbuf.writeUInt32LE(name.length+1,level_structure.header.name_size_start)
        newbuf.writeUInt32LE(name.length+level_structure.header.name_section_size_add,level_structure.header.name_section_size_start)
        return newbuf
    }
    setGrid(buf:Buffer,grid:CellGrid){
        const detail = new LevelDetails(buf)

        const buffer_before = Buffer.from(buf.slice(0,detail.grid.offset))
        const buffer_after = buf.slice(detail.pgrid.offset+detail.pgrid.buf.length) // to the end

        // now we need to insert the grid size into the before buffer
        buffer_before.writeUInt32LE(grid.rows*grid.cols*5+8,level_structure.header.cell_data_field_size+detail.name.length)
        buffer_before.writeUInt32LE(grid.rows,level_structure.header.row_num_start+detail.name.length)
        buffer_before.writeUInt32LE(grid.cols,level_structure.header.col_num_start+detail.name.length)

        // we also need an empty post-grid
        const pgrid = Buffer.alloc(grid.cols*grid.rows,0)

        // check consistency:
        if(grid.buf.length - grid.cols*grid.rows*4){
            console.warn('WARNING: grid buffer is inconsistent.')
        }
        const final_buf = Buffer.concat([buffer_before,grid.buf,pgrid,buffer_after])

        // this has no effect if the structure is good.
        const check = new LevelDetails(final_buf)
